...
import { clone, isNumber, replaceText } from "../util/util.js"
...
export function FarmControl(farmRepository: IFarmRepository) {
    ...
    const processFarm = ({ modelTables, settings, process }: { modelTables: ITableModel[], settings: ISettingsGeneral, process: TFarmProcess[] }) => {
        ...
        const PROCESS = {
            ...
            "deadline+D": () => {
                ...
                const { logs: logsInsertValuesDMoreOne } = insertValuesDMoreOne({ tableModel: modelTableFarm, tableBase: clone(modelTableFarm), settings })
    ...
    // Repo
    const getData: () => IFarm = () => {
        return clone(farmRepository.data)
    }
    ...
    const getTable: (props: { code: TTableCode }) => { table: TTable, code: TTableCode, headers: THeader[], name: string, index: number, }[] = (props: { code: TTableCode }) => {
        return clone(farmRepository.getTable(props))
    }
    ...
    function getHeaders({ tableModel, code, types = [] }: { code?: TTableCode, tableModel?: { table: TTable, code?: TTableCode, headers: THeader[] }, types?: THeaderCellType[] }) {
        if (code) {
            return clone(farmRepository.getHeaders({ code, types }))
        }
    ...
    const getSettings: () => ISettingsFarm = () => {
        return clone(farmRepository.getSettings())
    }
