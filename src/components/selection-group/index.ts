function SelectionGroupComponent(form: HTMLElement, props: {
  ...
  const GET_INPUT_VALUE = {
    ...
    file: (el: HTMLInputElement) => { return el.files ? el.files[0] || null : null },
  }
  ...
  const getDataList = () => {
    ...
    baseEl.forEach((_base) => {
      const listBases: { values?: TFormResult[][], subMenu?: TFormResult[][] }[] = []
      ...
      props.pathsValue.forEach((_path) => {
        ...
        if (!_path.children) { listBases.push({ values }) }
        else { listBases.push({ subMenu: values }) }
      })
